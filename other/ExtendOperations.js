...
import {
	...
	getOperationAction,
	SKIP_TRI, INVERT_TRI,
} from './ExtendOperationsUtils.js';
import { getTriCount } from '../node_modules/three-bvh-csg/src/core/utils/geometryUtils.js';
import { HOLLOW_INTERSECTION, HOLLOW_SUBTRACTION, SECTION } from './ExtendConstants.js';
import { isTriDegenerate } from '../node_modules/three-bvh-csg/src/core/utils/triangleUtils.js';
...
let planeCenter = new Vector3(0,0,0);
let planeNormal = new Vector3(0,0,1);
const _barycoordTri = new Triangle();
const _VEMap = new VEMap();
const _InsertEdgeMap = new InsertEdgeMap();
const _attr = [];
const _actions = [];
...
export function setPlaneParam(plane_Center, plane_Normal)
{
	planeCenter =  plane_Center.clone();
	planeNormal = plane_Normal.clone();
}
...
export function performOperation(
	...
	const nonHollow = operations
		.findIndex( op => op !== HOLLOW_INTERSECTION && op !== HOLLOW_SUBTRACTION && op !== SECTION) !== - 1;
...
export function getCutBorder()
{
	const vemap = _VEMap.clone();
	const insertEdgeMap = _InsertEdgeMap.clone();
	return {vemap, insertEdgeMap};
}
...
function performSplitTriangleOperations(
	...
	_translateMatrix.copy(b.matrixWorld);
	...
	for ( let i = 0, l = splitIds.length; i < l; i ++ ) {
		...
		for ( let ib = 0, l = intersectingIndices.length; ib < l; ib ++ ) {
			...
			splitter.splitByTriangle( _triB, _VEMap, _InsertEdgeMap, _translateMatrix);
		}
		...
		for ( let ib = 0, l = triangles.length; ib < l; ib ++ ) {
			...
			const clippedTri = triangles[ ib ];//三角形都在b的局部坐标
			...
			for ( let o = 0, lo = operations.length; o < lo; o ++ ) {
				let isPos = true;
				if(!invert  && operations[ o ] === SECTION)
				{
					if(clippedTri.a.clone().sub(planeCenter).dot(planeNormal)<0 
					||clippedTri.b.clone().sub(planeCenter).dot(planeNormal)<0 
					||clippedTri.c.clone().sub(planeCenter).dot(planeNormal)<0 )
					{
						isPos = false;
					}
				}
...
function performWholeTriangleOperations(
	...
	while ( traverseSet.size > 0 ) {
		...
		for ( let o = 0, lo = operations.length; o < lo; o ++ ) {
			...
			if(!invert && operations[ o ] === SECTION)
			{
				if(_tri.a.clone().sub(planeCenter).dot(planeNormal)<0 
				||_tri.b.clone().sub(planeCenter).dot(planeNormal)<0 
				||_tri.c.clone().sub(planeCenter).dot(planeNormal)<0 )
				{
					isPos = false;
				}
			}
