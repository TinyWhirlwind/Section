...
export class TriangleSplitter {
	...
	splitByTriangle( triangle, _VEMap, _InsertEdgeMap, _translateMatrix) {
		...
		if ( Math.abs( 1.0 - Math.abs( _triangleNormal.dot( normal ) ) ) < PARALLEL_EPSILON ) {
			...
			for ( let i = 0; i < 3; i ++ ) {
				...
				this.splitByPlane( _plane, triangle, _VEMap, _InsertEdgeMap, _translateMatrix);
			}
	...
	splitByPlane( plane, clippingTriangle, _VEMap, _InsertEdgeMap, _translateMatrix) {
		...
		for ( let i = 0, l = triangles.length; i < l; i ++ ) {
			...
			for ( let t = 0; t < 3; t ++ ) {
				...
				if ( didIntersect && ! ( _vec.distanceToSquared( _edge.start ) < SQUARED_EPSILON ) ) {
					...
					if ( _vec.distanceToSquared( _edge.end ) < SQUARED_EPSILON ) {
			...
			if ( ! coplanarEdge && intersects === 2 && _foundEdge.distance() > COPLANAR_EPSILON ) {
				...
				if ( vertexSplitEnd !== - 1 ) {
					...
					if ( ! isTriDegenerate( nextTri ) ) {
						...
						const vid0 = _VEMap.insertPoint(_foundEdge.end.clone().applyMatrix4(_translateMatrix));
						const vid1 = _VEMap.insertPoint(_foundEdge.start.clone().applyMatrix4(_translateMatrix));
						if(arr[ otherVert2 ].sub(planeCenter).dot(planeNormal)>0)
						{
							_InsertEdgeMap.insertEdge(vid0, vid1);
						}
						else
						{
							_InsertEdgeMap.insertEdge(vid1, vid0);
						}
					}
					...
					const vid0 = _VEMap.insertPoint(_foundEdge.end.clone().applyMatrix4(_translateMatrix));//在世界坐标中
					const vid1 = _VEMap.insertPoint(_foundEdge.start.clone().applyMatrix4(_translateMatrix));
					if(arr[ singleVert ].clone().sub(planeCenter).dot(planeNormal)>0)//点和平面都在b的局部坐标中
					{
						_InsertEdgeMap.insertEdge(vid0, vid1);
					}
					else
...
export function setCutPlane(plane_Center,plane_Normal)
{
	planeCenter = plane_Center;
